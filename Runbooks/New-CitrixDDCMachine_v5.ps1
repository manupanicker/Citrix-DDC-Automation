# v5: Designed for execution directly in an Azure Automation sandbox.
# Authentication uses the Automation Account Managed Identity through
# Connect-AzAccount / Get-AzAccessToken and Key Vault REST API.
...
if ([string]::IsNullOrWhiteSpace($KeyVaultName)) {
    throw "Automation variable 'KeyVaultName' is missing or empty."
}
...
Write-Section "Retrieving DDC Credential From Key Vault"
...
if ([string]::IsNullOrWhiteSpace($KeyVaultToken)) {
    throw "Failed to obtain Key Vault access token from Azure Automation Managed Identity."
}
...
try {
    $CredentialData = $SecretValue | ConvertFrom-Json -ErrorAction Stop

    if ([string]::IsNullOrWhiteSpace($CredentialData.Username) -or
        [string]::IsNullOrWhiteSpace($CredentialData.Password)) {
        throw "JSON credential fields are missing."
    }

    $AdminUsername = $CredentialData.Username
    $AdminPasswordPlainText = $CredentialData.Password
}
catch {
...
$AdminPasswordSecure = ConvertTo-SecureString `
...
Write-Output "Credential username: $AdminUsername"
...
Write-Section "Preparing Domain Join Credential"

$DomainUsername = $AdminUsername
$DomainPassword = $AdminPasswordPlainText

Write-Output "Domain credential username: $DomainUsername"
