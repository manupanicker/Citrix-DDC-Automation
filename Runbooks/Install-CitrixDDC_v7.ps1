...
try {
    ...
    while ($Preflight.PendingReboot) {
        ...
        # Re-check the pending reboot state. If another reboot is still
        # required, the loop will perform it, up to the six-reboot limit.
        ...
        if (-not $Preflight.PendingReboot) {
            Write-Output "Pending reboot cleared successfully."
        }
        else {
            Write-Output "Target still reports a pending reboot. Automatic reboot loop will continue."
        }
    }
    ...
    # --------------------------------------------------------
    # Handle any reboot still required after installation
    # --------------------------------------------------------

    $PostInstallPending = Invoke-Command -Session $Session -ScriptBlock {
    ...
    while ($PostInstallPending) {

        $RebootCount++

        if ($RebootCount -gt $MaxReboots) {
            throw "Maximum automatic reboot limit of $MaxReboots has been reached. Target server still reports a pending reboot after DDC installation."
        }

        Write-Output ""
        Write-Output "============================================================"
        Write-Output "POST-INSTALL REBOOT REQUIRED"
        Write-Output "============================================================"
        Write-Output "Automatic reboot: $RebootCount of $MaxReboots"
        Write-Output "Rebooting target server..."
        ...
        if ($PostInstallPending) {
            Write-Output "Target still reports a pending reboot. Another automatic reboot will be attempted."
        }
        else {
            Write-Output "Post-install pending reboot cleared successfully."
        }
    }
