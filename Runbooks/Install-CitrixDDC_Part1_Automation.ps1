...
Write-Output "Windows Prerequisites"
Write-Output "============================================================"

# ------------------------------------------------------------
# Automation variables - keep identical to existing DDC/Licensing
# automation
# ------------------------------------------------------------
...
try {
    ...
    Write-Output ""
    Write-Output "============================================================"
    Write-Output "DDC PART 1 PRE-FLIGHT"
    Write-Output "============================================================"
    ...
    if ($Preflight.IsDomainController) {
        throw "DDC target is a Domain Controller. Citrix Delivery Controller must not be installed on a Domain Controller."
    }

    # --------------------------------------------------------
    # Install DDC prerequisites
    # Exact feature list from supplied YAML
    # --------------------------------------------------------
    ...
    $FeatureResult = Invoke-Command -Session $Session -ScriptBlock {
    ...
    Write-Output "Feature installation completed."
    Write-Output "Success      : $($FeatureResult.Success)"
    Write-Output "RestartNeeded : $($FeatureResult.RestartNeeded)"
    Write-Output "ExitCode      : $($FeatureResult.ExitCode)"

    if (-not $FeatureResult.Success) {
        throw "DDC prerequisite installation failed. ExitCode: $($FeatureResult.ExitCode)"
    }

    # --------------------------------------------------------
    # Reboot if prerequisites require it
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Verify prerequisites after reboot
    # --------------------------------------------------------
    ...
    $Verification | Format-Table -AutoSize | Out-String | Write-Output
    ...
    if ($Missing) {
        throw "One or more DDC prerequisites are not installed: $($Missing.Name -join ', ')"
    }

    Write-Output ""
    Write-Output "============================================================"
    Write-Output "DDC PART 1 COMPLETED SUCCESSFULLY"
    Write-Output "============================================================"
}
finally {
