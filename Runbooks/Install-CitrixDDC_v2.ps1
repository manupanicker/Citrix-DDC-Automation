    ...
    if ($ExistingDDC) {
        ...
        $InstallResult = Invoke-Command -Session $Session -ScriptBlock {
            ...
            while (-not $Process.HasExited) {
                ...
                $ElapsedSeconds += $HeartbeatSeconds

                if (-not $Process.HasExited) {
                    Write-Output "Citrix installer still running... $ElapsedSeconds seconds elapsed."
                }
            }

            $Process.Refresh()

            Write-Output "Citrix installer process exited."
            Write-Output "Citrix installer exit code : $($Process.ExitCode)"
