        ...
        while (-not $InstallCompleted) {
            ...
            try {
                $RemoteStatus = Invoke-Command -Session $Session -ScriptBlock {
                    ...
                    # The exit-code file is the authoritative installation state.
                    # Get-Content can return NULL while the file is being created or
                    # written, so never call .Trim() directly on a NULL value.
                    if (Test-Path -LiteralPath $ResultFile) {
                    ...
                    if (-not $Result.Completed) {
                        # The wrapper PID is only used as a secondary health check.
                        # Do not call methods on the Process object; a remote process
                        # object is not a persistent handle across WinRM calls.
                        ...
                        if ($null -ne $Process) {
                            ...
                            # Wrapper has disappeared without writing a valid exit code.
                            $Result.Failed = $true
                        }
                    }
            ...
            if ($RemoteStatus -and $RemoteStatus.Failed) {
                throw "Citrix installer wrapper process ended without writing a valid exit code to $InstallResultFile. Check $InstallLogFile on the target server."
            }
