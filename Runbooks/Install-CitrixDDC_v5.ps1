...
try {
    ...
    if ($ExistingDDC) {
        ...
        $InstallState = Invoke-Command -Session $Session -ScriptBlock {
            ...
            Remove-Item -LiteralPath $StartedFile,$PidFile -Force -ErrorAction SilentlyContinue

            Write-Output "Installer : $Installer"
            Write-Output "Log       : $LogDirectory"
            Write-Output "Result    : $ResultFile"
            Write-Output "Launcher  : $LauncherScript"
            ...
            Write-Output "Creating target-side Citrix installer launcher..."
...
'@
            ...
            Write-Output "Starting target-side launcher..."
            ...
            Write-Output "Launcher Process ID : $($Launcher.Id)"

            # Give the detached launcher a moment to create its started marker.
            ...
            if (Test-Path -LiteralPath $PidFile) {
                ...
                if ($null -ne $RawPid -and $RawPid.Trim() -match '^\d+$') {
                    $CitrixPid = [int]$RawPid.Trim()
                }
            }

            if (-not $Started) {
                throw "Target-side Citrix launcher did not create the started marker. The Citrix installer was not confirmed as launched."
            }

            [pscustomobject]@{
                LauncherProcessId = $Launcher.Id
                CitrixProcessId = $CitrixPid
        ...
        $LauncherPid = [int]$InstallState.LauncherProcessId
        $CitrixPid = if ($InstallState.CitrixProcessId) { [int]$InstallState.CitrixProcessId } else { $null }
        ...
        Write-Output "Citrix installer launcher started successfully."
        Write-Output "Launcher Process ID : $LauncherPid"

        if ($CitrixPid) {
            Write-Output "Citrix installer Process ID : $CitrixPid"
            Write-Output "Citrix installer launch confirmed."
        }
        else {
            Write-Output "Citrix installer launch marker confirmed; installer PID not yet available."
        }
        ...
        while (-not $InstallCompleted) {
            ...
            try {
                $RemoteStatus = Invoke-Command -Session $Session -ScriptBlock {
                    param($LauncherProcessId, $CitrixProcessId, $StartedFile, $PidFile, $ResultFile)
                    ...
                    if (Test-Path -LiteralPath $ResultFile) {
                        ...
                        if ($null -ne $RawExitCode) {
                            ...
                            if (-not [string]::IsNullOrWhiteSpace($RawExitCode) -and
                                $RawExitCode -match '^-?\d+$') {
                    ...
                    if (-not $Result.Completed) {
                        $CurrentCitrixPid = $CitrixProcessId

                        if ($null -eq $CurrentCitrixPid -and (Test-Path -LiteralPath $PidFile)) {
                            $RawPid = Get-Content -LiteralPath $PidFile -Raw -ErrorAction SilentlyContinue
                            if ($null -ne $RawPid -and $RawPid.Trim() -match '^\d+$') {
                        ...
                        if (-not $Result.Running -and $null -ne $LauncherProcessId) {
                        ...
                        if (-not $Result.Running) {
                            # Do not immediately fail if the PID has disappeared. The
                            # launcher may be finalizing the exit-code file.
                ...
                } -ArgumentList $LauncherPid, $CitrixPid, $InstallStartedFile, $InstallPidFile, $InstallResultFile -ErrorAction Stop
            }
            catch {
            ...
            if ($RemoteStatus -and $RemoteStatus.Failed) {
                throw "Citrix installer launcher ended without writing a valid exit code to $InstallResultFile. Check $InstallLogFile on the target server."
            }

            if ($RemoteStatus -and $RemoteStatus.Running) {
                if ($RemoteStatus.CitrixProcessRunning) {
                    Write-Output "Citrix installer still running... $InstallElapsedSeconds seconds elapsed."
                }
                else {
                    Write-Output "Citrix installer launcher still running; waiting for installer state... $InstallElapsedSeconds seconds elapsed."
                }
            }
            else {
